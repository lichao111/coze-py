...
class LocalPluginMocker(object):
    ...
    def robot_navigate(target_json: str):
        ...
        global robot_nivagation_status
        robot_nivagation_status = "started"
        return "success"
    ...
    def robot_cancel(task):
        ...
        if target == 1:
            ...
            global robot_nivagation_status
            robot_nivagation_status = "stopped"
        elif target == 3:
...
if __name__ == "__main__":
    ...
    mock_input_file = os.path.join(os.path.dirname(__file__), "test_data/input_navigation.json")
    inputs = json.load(open(mock_input_file, "r", encoding="utf-8"))
    logging.info(f" inputs number: {len(inputs)}")

    result_json = os.path.join(os.path.dirname(__file__), "test_data/result_navigation.json")
    result = {}
