...
class LocalPluginMocker(object):
    ...
    @staticmethod
    def robot_rechargeing():
        # mock sleeping
        logging.info("rechargeing started")
        #time.sleep(1)
        global robot_rechargeing_status
        robot_rechargeing_status = "started"
        return "success"
    ...
    def robot_cancel(task):
        ...
        if target == 1:
            ...
            global robot_rechargeing_status
            robot_rechargeing_status = "stopped"
        elif target == 2:
    ...
    def get_function(name: str):
        func_dict = {
            ...
            "robot_rechargeing": LocalPluginMocker.robot_rechargeing,
            "robot_cancel": LocalPluginMocker.robot_cancel,
        }
        if name in func_dict:
            return func_dict[name]
        else:
            # 可以选择抛出自定义异常，或者返回 None
...
if __name__ == "__main__":
    ...
    mock_input_file = os.path.join(os.path.dirname(__file__), "test_data/input_rechargeing.json")
    inputs = json.load(open(mock_input_file, "r", encoding="utf-8"))
    logging.info(f" inputs number: {len(inputs)}")

    result_json = os.path.join(os.path.dirname(__file__), "test_data/result_rechargeing.json")
    result = {}
    ...
    for input in tqdm(inputs, desc="Processing", unit="input"):
        ...
        if robot_rechargeing_status != "started":
