...
class LocalPluginMocker(object):
    ...
    @staticmethod
    def robot_tracking(target_json: str):
        # mock sleeping
        target = json.loads(target_json).get("target")
        logging.info(f"tracking started target: {target}")
        #time.sleep(1)
    ...
    def get_function(name: str):
        return {
            ...
            "robot_tracking": LocalPluginMocker.robot_tracking,
...
if __name__ == "__main__":
    ...
    mock_input_file = os.path.join(os.path.dirname(__file__), "test_data/input_tracking.json")
    inputs = json.load(open(mock_input_file, "r", encoding="utf-8"))
    logging.info(f" inputs number: {len(inputs)}")

    result_json = os.path.join(os.path.dirname(__file__), "test_data/result_tracking.json")
    result = {}
    ...
    for input in tqdm(inputs, desc="Processing", unit="input"):
        ...
        if robot_tracking_status != "started":
        ...
        if robot_tracking_status != "stopped":
    ...
    result["success_rate"] = (total_count - fail_count) / total_count
    json.dump(result, open(result_json, "w", encoding="utf-8"), ensure_ascii=False, indent=4)